...
use super::state::{pending_kv_response_len, take_pending_kv_response};
...
pub(super) fn register(
    ...
    linker.func_wrap(
        ...
        move |_caller: wasmtime::Caller<'_, wasmtime_wasi::preview1::WasiP1Ctx>| -> i32 {
            pending_kv_response_len(&len_shared)
        },
    )?;
    ...
    linker.func_wrap(
        ...
        move |mut caller: wasmtime::Caller<'_, wasmtime_wasi::preview1::WasiP1Ctx>,
            ...
            let Some(data) = take_pending_kv_response(&shared) else {
                return -1;
            };
            let write_len = std::cmp::min(data.len(), len as usize);
            if let Err(error) = write_memory_from_caller(&mut caller, ptr, &data[..write_len]) {
