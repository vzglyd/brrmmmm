...
use brrmmmm::controller::{MissionInspection, inspect_module_contract, validate_module_inspection};

use crate::cli::OutputFormat;

use super::output::print_table;

pub fn cmd_validate(wasm_path: &Path, output: OutputFormat) -> Result<()> {
    ...
    match output {
        OutputFormat::Text => print_validate_text(&inspection, &wasm_str)?,
        OutputFormat::Json => print_validate_json(&inspection)?,
        OutputFormat::Table => print_validate_table(&inspection)?,
    }

    Ok(())
}

fn print_validate_text(inspection: &MissionInspection, wasm_str: &str) -> Result<()> {
    ...
    print_describe_summary(inspection.describe.as_ref())?;
    print_import_export_summary(inspection);
    Ok(())
}

fn print_describe_summary(describe: Option<&MissionModuleDescribe>) -> Result<()> {
...
fn print_import_export_summary(inspection: &MissionInspection) {
...
fn print_validate_json(inspection: &MissionInspection) -> Result<()> {
...
fn print_validate_table(inspection: &MissionInspection) -> Result<()> {
    let rows = validate_rows(inspection)?;
    print_table(&rows);
    Ok(())
}

fn validate_rows(inspection: &MissionInspection) -> Result<Vec<(&str, String)>> {
    let describe = inspection.describe.as_ref();
    let mut rows = vec![
        ("valid", "✓".to_string()),
